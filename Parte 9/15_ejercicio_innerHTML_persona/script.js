...
//Preguntar nombre usuario
let nombre = prompt("Introduce nombre de usuario:");

//Primero recogemos el objeto body
...
//Buscar si esta en lista (!= undefined)
    //En caso de que este mostrar datos (Crear la web como indica)
if(personas[nombre] != undefined){
    div.innerHTML += `<img />`;
    document.querySelector("img").src = personas[nombre].picture;

    div.innerHTML += `<h1>Name: ${nombre}</h1>`;
    
    div.innerHTML += `<p>Age: ${personas[nombre].age}</p>`;
    div.innerHTML += `<p>Email: ${personas[nombre].email}</p>`;
    div.innerHTML += `<p>Address: ${personas[nombre].address}</p>`;
    
}
    //En caso de no estar notificar con un mensaje
else{
    div.innerHTML += `<p>No estoy definido</p>`;  
}
